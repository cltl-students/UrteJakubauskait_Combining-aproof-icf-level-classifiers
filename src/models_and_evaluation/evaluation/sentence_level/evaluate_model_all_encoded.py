"""
Evaluate fine-tuned regression models on an evaluation set for a single,
hardcoded domain.
...
$ python evaluate_model_all_encoded.py --clas_unit note
"""
...
def evaluate(test_pkl,
    ...
    result, model_outputs, wrong_predictions = model.eval_model(dom_df)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datapath', default='data_expr_sept', help='Key listed in config.ini')
    ...
    parser.add_argument('--model_name', default='levels_all_tokens_sents')
    ...
    # prepend domain tokens before saving
    combined_test_df['text'] = combined_test_df.apply(lambda row: f"{domain_token_map[row['domain']]} {row['text']}", axis=1)
    ...
    model_name = PROJECT_ROOT / args.modelpath / f"levels_all_tokens_sents_combined_all"
    output_dir = model_name / f'eval_{args.eval_on}_all_tokens_combined_all'

    print(f"Evaluating {model_name} on combined test data at {test_pkl}")
    evaluate(test_pkl,
             args.model_type,
             str(model_name),
             output_dir)
