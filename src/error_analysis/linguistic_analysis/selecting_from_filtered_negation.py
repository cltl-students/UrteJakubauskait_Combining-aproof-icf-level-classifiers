...
negation words from multiple filtered training datasets and combines them 
...
'selected_negation_sentences.csv'.
...
output_csv = "selected_negation_sentences.csv"
sample_size = 12

negation_words = ["niet", "geen", "nimmer", "nooit", "geenszins",
                  "niets", "niks", "geen enkel ding", "helemaal niets",
                  "geen", "niet één", "nul", "geeneen"]
...
for filename in os.listdir(input_folder):
    ...
    mask = df["text"].astype(str).str.lower().apply(lambda s: any(neg in s for neg in negation_words))
    neg_df = df[mask]

    selected_rows = neg_df.sample(n=min(sample_size, len(neg_df)), random_state=42)
